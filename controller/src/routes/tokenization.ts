...
export const registerTokenizationRoutes = (app: Hono, context: AppContext): void => {
  app.post("/v1/tokenize", async (ctx) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:${context.config.inference_port}/tokenize`, {
        method: "POST",
  ...
  app.post("/v1/detokenize", async (ctx) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:${context.config.inference_port}/detokenize`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
  ...
  app.post("/v1/tokenize-chat-completions", async (ctx) => {
    ...
    try {
      ...
      if (tools.length > 0) {
        ...
        const toolsResponse = await fetch(`http://127.0.0.1:${context.config.inference_port}/tokenize`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
Title:`;
      ...
      const response = await fetch("http://127.0.0.1:4100/v1/chat/completions", {
        method: "POST",
        headers: {
