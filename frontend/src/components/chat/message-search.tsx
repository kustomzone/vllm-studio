      ...
      <div className="flex items-center gap-2 p-3 border-b border-(--border) bg-(--accent)/30">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 bg-(--background) border border-(--border) rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-(--primary) focus:border-transparent"
          />
          {searchQuery && (
            <button
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-(--background) border border-(--border) rounded-lg text-sm hover:bg-(--accent) transition-colors"
          >
            <Filter className="h-3.5 w-3.5 text-[#9a9590]" />
            <span className="capitalize hidden sm:inline">{filterType}</span>
            <ChevronDown
              className={`h-3 w-3.5 text-[#9a9590] transition-transform ${isFilterOpen ? "rotate-180" : ""}`}
            />
          </button>
          ...
          <AnimatePresence>
            {isFilterOpen && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-2 w-40 bg-(--background) border border-(--border) rounded-lg shadow-lg z-50 overflow-hidden"
              >
