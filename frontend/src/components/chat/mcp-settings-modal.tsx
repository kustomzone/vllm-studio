...
import { X, Globe, Plus, Trash2, AlertCircle, Server, Terminal } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-(--card) border border-(--border) rounded-lg w-full max-w-lg max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {error && (
            <div className="flex items-center gap-2 px-3 py-2 bg-(--error)/10 border border-(--error)/20 rounded text-sm text-(--error)">
          ...
          {localServers.map((server) => (
            <div
              key={server.name}
              className="flex items-center justify-between px-3 py-2 bg-(--accent) rounded-lg border border-(--border)"
            >
              <div className="flex items-center gap-3">
                <button
                  onClick={() => toggleServer(server.name)}
                  className={`w-8 h-5 rounded-full transition-colors ${
                    server.enabled ? "bg-(--success)" : "bg-(--muted)"
                  }`}
                >
                  <div
              ...
              <button
                ...
                className="p-1.5 rounded hover:bg-(--error)/20 text-[#9a9590] hover:text-(--error) transition-colors"
              >
          ...
          {localServers.length > 0 && (
            <div className="text-xs text-[#9a9590] bg-(--background) p-3 rounded-lg border border-(--border)">
          ...
          {/* Add Server Form */}
          {isAdding ? (
            <div className="space-y-3 p-3 bg-(--accent) rounded-lg border border-(--border)">
              <input
                type="text"
                placeholder="Server name (e.g., brave-search)"
                value={newServer.name}
                onChange={(e) => setNewServer({ ...newServer, name: e.target.value })}
                className="w-full px-3 py-2 text-sm bg-(--background) border border-(--border) rounded focus:outline-none focus:border-(--foreground)"
              />
              <input
                type="text"
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={addEnvPair} className="text-xs text-blue-500 hover:underline">
                    + Add variable
                  </button>
                </div>
                {envPairs.map((pair, index) => (
                  <div key={index} className="flex gap-2">
                    ...
                    <button
                      ...
                      className="p-1 text-[#9a9590] hover:text-(--error)"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 px-3 py-1.5 text-sm border border-(--border) rounded hover:bg-(--background) transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={addServer}
                  disabled={saving}
                  className="flex-1 px-3 py-1.5 text-sm bg-(--foreground) text-(--background) rounded hover:opacity-90 transition-opacity disabled:opacity-50"
                >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm border border-dashed border-(--border) rounded-lg hover:bg-(--accent) transition-colors"
            >
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-(--border)">
          ...
          <button
            onClick={handleSave}
            className="px-4 py-1.5 text-sm bg-(--foreground) text-(--background) rounded hover:opacity-90 transition-opacity"
          >
            Save
          </button>
        </div>
