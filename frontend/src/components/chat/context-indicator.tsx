    ...
    <div className="relative">
      ...
      {variant === "icon" ? (
        <button
          ...
          className={`relative h-8 w-8 rounded-full border border-(--border)/40 text-[#9a9590] hover:text-[#c9a66b] hover:border-[#c9a66b]/40 transition-colors ${showDetails ? "text-[#c9a66b]" : ""}`}
          title={iconLabel}
        ...
        <button
          onClick={() => setShowDetails(!showDetails)}
          className="flex items-center gap-3 px-3 py-1.5 rounded-md transition-all hover:bg-(--accent)/10 border border-(--border)/30"
        ...
        >
          ...
          <span className={`text-xs font-mono ${colors.text}`}>{percentage}%</span>
          {isWarning && <AlertTriangle className="h-3.5 w-3.5 text-[#c9a66b] animate-pulse ml-1" />}
          {showDetails ? (
      ...
      {/* Details popup */}
      {showDetails && (
        <div className="absolute bottom-full right-0 mb-2 w-80 bg-(--background) border border-(--border)/50 rounded-lg shadow-xl z-50 overflow-hidden">
          {/* Header */}
            ...
            <>
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="bg-(--accent)/5 p-2 rounded border border-(--border)/20">
                  ...
                  <div className="bg-(--accent)/5 p-2 rounded border border-(--border)/20">
                    ...
                    <div className="text-[#d0c8c0] font-mono">
                ...
                {/* Warning message */}
                {!canSendMessage && (
                  <div className="flex items-center gap-2 p-2 bg-(--error)/10 rounded text-sm text-(--error) border border-(--error)/20">
                ...
                {stats.totalCompactions > 0 && (
                  <div className="text-xs text-[#9a9088] border-t border-(--border)/30 pt-3">
                    Total compactions: {stats.totalCompactions} (
                    {formatTokenCount(stats.totalTokensCompacted)} tokens freed)
                  </div>
                )}
              </div>
              ...
              <div className="p-4 border-t border-(--border)/30 flex gap-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-(--accent)/20 text-[#d0c8c0] rounded-lg hover:bg-(--accent)/30 disabled:opacity-50 disabled:cursor-not-allowed text-sm transition-colors"
                >
                ...
                <button
                  ...
                  disabled={stats.messagesCount <= config.preserveRecentMessages}
                  className="flex items-center justify-center gap-2 px-3 py-2 border border-(--border)/30 text-[#9a9088] rounded-lg hover:bg-(--accent)/10 disabled:opacity-50 disabled:cursor-not-allowed text-sm transition-colors"
...
function HistoryPanel({ history }: { history: CompactionEvent[] }) {
  const { formatTokenCount } = useContextManagement();
  const sortedHistory = useMemo(() => [...history].reverse().slice(0, 20), [history]);

  if (history.length === 0) {
    return <div className="p-4 text-center text-[#9a9088] text-sm">No compaction history</div>;
  }
  ...
  return (
    <div className="max-h-64 overflow-auto">
      {sortedHistory.map((event) => (
        <div
          key={event.id}
          className="p-3 border-b border-(--border)/30 last:border-0 hover:bg-(--accent)/5 transition-colors"
        >
          <div className="flex items-center justify-between text-sm">
            <span className="text-[#d0c8c0]">{event.strategy}</span>
            <span className="text-xs text-[#9a9088]">{event.timestamp.toLocaleTimeString()}</span>
          </div>
