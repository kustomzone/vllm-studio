...
export default function Dashboard() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setCurrentProcess(statusData.running_process);

      if (statusData.running_process) {
        // Find running recipe from recipes list (which has status), or match by id
        const runningRecipe = recipesList.find((r: RecipeWithStatus) => r.status === 'running');
        const matchedRecipe = statusData.matched_recipe
          ? recipesList.find((r: RecipeWithStatus) => r.id === statusData.matched_recipe?.id)
          : null;
        const recipe = runningRecipe || matchedRecipe;
        setCurrentRecipe(recipe || null);

        if (recipe) {
          const logsData = await api.getLogs(recipe.id, 50).catch(() => ({ logs: [] }));
