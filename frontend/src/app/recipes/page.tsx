...
function RecipesContent() {
  ...
  const loadData = useCallback(async () => {
    const [modelsData, recipesData, statusData] = await Promise.all([
      ...
      api.getStatus().catch(() => ({ running_process: null as ProcessInfo | null, matched_recipe: null as Recipe | null })),
    ]);

    setModels(modelsData.models || []);
    const recipesList = recipesData.recipes || [];
    setRecipes(recipesList);
    setRunningProcess(statusData.running_process || null);
    const running = recipesList.find((r) => r.status === 'running')?.id || statusData.matched_recipe?.id || null;
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-[var(--muted-foreground)]">
            {runningProcess
              ? `Running: ${runningRecipeId || runningProcess.model_path.split('/').pop()} (PID ${runningProcess.pid})`
              : 'No model running'}
          </div>
        </div>
