...
import { UsageModalProps } from "@/app/chat/components/types";
      ...
      <div className="bg-(--card) border border-(--border) rounded-lg w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="border border-(--border) rounded-lg overflow-hidden">
            <div className="px-3 py-2 bg-(--accent) text-xs font-medium text-[#b0a8a0]">
            ...
            <div className="max-h-64 overflow-y-auto">
              {messages
                ...
                .reverse()
                .map((m) => {
                  const inTok = (m.request_total_input_tokens ??
                    m.request_prompt_tokens ??
                    null) as number | null;
                  ...
                  return (
                    <div
                      key={m.id}
                      className="px-3 py-2 border-t border-(--border) flex items-center gap-3"
                    >
...
export function ExportModal({ isOpen, onClose, onExportMarkdown, onExportJson }: ExportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-(--card) border border-(--border) rounded-lg w-full max-w-lg overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-(--border)">
          <div className="flex items-center gap-2">
