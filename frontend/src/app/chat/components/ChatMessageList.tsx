...
import { ChatMessageListProps, RenderMessage } from "@/app/chat/components/types";
...
export function ChatMessageList({
  ...
  const normalizedMessages = useMemo(() => {
    return messages.map((msg) => {
      ...
      const normalized =
        isAssistant && rawContent ? markdownParser.normalizeForRender(rawContent) : rawContent;
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {message.images.map((base64, i) => (
                <Image
                  ...
                  width={140}
                  height={140}
                  className="max-w-[140px] max-h-[140px] rounded-lg border border-(--border) h-auto w-auto"
                  unoptimized
                />
              ))}
            </div>
    ...
    return (
      <div id={`message-${message.id}`} className="max-w-4xl mx-auto w-full px-4 md:px-6 py-2">
        <div className="relative group max-w-[88%] md:max-w-[80%] mr-auto">
          <div className="text-[10px] uppercase tracking-[0.2em] text-[#9a9590] mb-1">
            Assistant
          </div>
          <MessageRenderer
            content={content}
          ...
          />
          {renderDebug && (
            <details className="mt-2 rounded-lg border border-dashed border-(--border) bg-(--card)/40 px-3 py-2 text-xs text-[#b8b4ad]">
              ...
              <div className="mt-2 space-y-2">
                <div>
                  ...
                  <pre className="mt-1 whitespace-pre-wrap break-words rounded border border-(--border)/50 bg-(--card)/70 p-2 font-mono text-[11px] text-[#d8d4cd]">
                    {message.rawContent ?? ""}
                  </pre>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-[0.18em] text-[#7a756f]">
                    Normalized
                  </div>
                  <pre className="mt-1 whitespace-pre-wrap break-words rounded border border-(--border)/50 bg-(--card)/70 p-2 font-mono text-[11px] text-[#d8d4cd]">
          ...
          >
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="p-1.5 rounded-full border border-transparent hover:border-(--border) text-[#8a8580] hover:text-[#c9a66b]"
                title="Copy"
                aria-label="Copy"
              >
                {showCopied ? (
              ...
              >
                {bookmarked ? (
                  <BookmarkCheck className="h-3.5 w-3.5 text-(--link)" />
                ) : (
