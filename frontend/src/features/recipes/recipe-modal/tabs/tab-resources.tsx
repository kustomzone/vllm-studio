...
export function RecipeModalTabResources({
  ...
  const showParallelism = capabilities.parallelism !== "none";
  const showGpuSection = capabilities.gpuMemoryUtil || capabilities.visibleDevices;
        ...
        <FormSection icon={<Cpu className="h-4 w-4" />} title="GPU">
          {capabilities.gpuMemoryUtil ? (
            <FormField
              ...
              description={
                capabilities.backend === "sglang"
                  ? "Maps to SGLang --mem-fraction-static."
                  : undefined
              }
            >
