...
- ⚠️ **Reload mid-stream on a NEW chat loses the turn** (root cause pinpointed): the URL stays `?…&new=<nonce>` after a turn starts — it never transitions to `?…&session=<piSessionId>`. `handlePiSessionIdChange` (chat-pane.tsx:751) only copies prefs + `notifySessionsChanged`; it does NOT update the URL. So reloading restarts a fresh new chat and the in-progress turn vanishes from view (still runs server-side, recoverable via the sidebar once it settles). **Fix = on piSessionId assignment, `history.replaceState` the URL to `?project&session=<piSessionId>` (silent, doesn't trip the `useSearchParams` nav effect) + sync `lastHandledNavKey` so a later nav doesn't replay.** Deferred — this is the frontend half of Phase 3b / the session-identity area flagged as riskiest; warrants a dedicated careful change, not a loop microcommit.
