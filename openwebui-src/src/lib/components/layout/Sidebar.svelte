	...
	>
		...
		>
			...
			<div class="-mt-[0.5px]">
				...
				{#if $user?.role === 'admin'}
					<div class="">
						<Tooltip content={$i18n.t('vLLM Studio')} placement="right">
							<a
								class=" cursor-pointer flex rounded-xl hover:bg-gray-100 dark:hover:bg-gray-850 transition group"
								href="/vllm-studio"
								on:click={async (e) => {
					...
					{#if $user?.role === 'admin'}
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							...
							>
								...
								<div class="flex self-center translate-y-[0.5px]">
									<div class=" self-center text-sm font-primary">{$i18n.t('vLLM Studio')}</div>
								</div>
							</a>
						</div>
					{/if}
